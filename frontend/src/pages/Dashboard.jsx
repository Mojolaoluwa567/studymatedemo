...
import {
  BookOpen, Brain, TrendingUp, Trash2,
  Flame, Target, FileText, Zap, Crosshair,
} from "lucide-react";
import {
  LineChart,
  Line,
...
function buildInsights(stats, attempts) {
  ...
  if (stats.current_streak >= 2) {
    ...
    insights.push(
      "Study or take a quiz today to start a new streak."
    );
  }
  ...
  const handleDelete = async (doc) => {
    toast((t) => (
      <div className="flex flex-col gap-2">
        ...
        <p className="text-xs text-muted">Removes all quizzes, attempts, and history. Can't be undone.</p>
        <div className="flex gap-2 mt-1">
          <button
            onClick={() => { toast.dismiss(t.id); doDelete(doc); }}
    ...
    <Layout>
      <div className="mb-8">
        ...
        <p className="text-muted text-sm">
          Upload course material, study it, then test yourself with a
          generated quiz.
        </p>
      </div>
      ...
      {loading ? (
        ...
        stats && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
            ...
            <StatCard label="Documents" value={stats.total_documents} icon={<FileText size={16} />} />
          </div>
        )
      )}
            ...
            <div style={{ width: "100%", height: 200 }}>
              <ResponsiveContainer>
                <LineChart data={chartData}>
                  ...
                  <XAxis dataKey="name" stroke="rgb(var(--color-muted))" fontSize={12} />
                  <YAxis
                  ...
                  <div className="flex items-center gap-2 shrink-0 ml-2">
                    <span
                      className={`font-mono text-xs px-1.5 py-0.5 rounded border capitalize ${
                        DIFFICULTY_COLORS[a.difficulty] || "border-border text-muted"
                      }`}
                    >
      ...
      <p className="text-sm text-muted mb-4">
        Everything you've uploaded. Study it, generate a quiz, track your progress, or focus on weak spots.
      </p>
          ...
          {documents.map((doc) => {
            ...
            >
              <div>
                ...
                <h3 className="font-semibold text-lg leading-snug line-clamp-2" title={doc.title}>
                  {doc.title}
                </h3>
              ...
              {ds ? (
                <div className="grid grid-cols-3 gap-2 text-center border border-border rounded-lg p-2">
                  <div>
                    ...
                    <p className="text-[10px] text-muted uppercase tracking-wide font-mono">Quizzes</p>
                  </div>
                  <div>
                    <p className="text-lg font-semibold text-accent">{ds.best}%</p>
                    <p className="text-[10px] text-muted uppercase tracking-wide font-mono">Best</p>
                  </div>
                  <div>
                    <p className="text-xs font-medium mt-1">{formatDate(ds.last)}</p>
                    <p className="text-[10px] text-muted uppercase tracking-wide font-mono">Last quiz</p>
                  </div>
                </div>
              ) : (
              ...
              <div className="grid grid-cols-2 gap-2 mt-auto">
                ...
                <button
                  onClick={async () => {
                    try {
                      const quiz = await api.post(`/documents/${doc.id}/weak-spots-quiz`);
